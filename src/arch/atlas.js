...
function paintSelection(canvas, detail, graph, selected) {
  canvas.querySelectorAll('.atlas-node').forEach((r) => r.setAttribute('data-selected', String(r.dataset.id === selected)));
  detail.innerHTML = renderDetail(graph.nodes.find((n) => n.id === selected) || null);
}
...
function paintLens(nav, canvas, detail, view, state, graph) {
  ...
  view.reset();
  paintSelection(canvas, detail, graph, state.selected);
...
export function initAtlas() {
  ...
  state.subscribe(() => {
    ...
    } else paintSelection(canvas, detail, graph, state.selected);
  });
  paintLens(nav, canvas, detail, view, state, graph);
  return state;
}
