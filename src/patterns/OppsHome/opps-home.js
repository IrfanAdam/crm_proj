...
const BARS = [30, 42, 36, 28, 48, 62, 40, 52, 34, 38, 44, 36, 50, 33, 39, 46, 31, 43, 55, 37, 41, 29, 47, 35, 58, 40, 32, 45, 38, 50, 34, 42, 49, 36, 31, 44, 53, 39, 46, 33, 41, 57, 35, 43, 30, 48, 37, 45, 52, 34, 40, 46, 32, 38, 51, 44];
/* outline gem = achievement, filled gem = highest so far (derived from the data) */
const PEAK = BARS.indexOf(Math.max(...BARS));
const HITS = BARS.map((v, i) => i).filter((i) => i !== PEAK && BARS[i] >= 52).slice(0, 5);
const TODAY = BARS.length - 1;
const GEM_OUT = `<svg class="opps__gem" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 1.8 14.2 6 8 14.2 1.8 6Z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg>`;
const GEM_FILL = `<svg class="opps__gem" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 1.8 14.2 6 8 14.2 1.8 6Z" fill="currentColor"/></svg>`;
const bars = BARS.map((h, i) => `<span class="opps__bar"><span class="opps__gem-slot">${i === PEAK ? GEM_FILL : HITS.includes(i) ? GEM_OUT : ""}</span><i class="${i === PEAK ? "on" : ""}" style="--h:${h}px"></i><span class="opps__dot-slot">${i === TODAY ? `<b class="opps__dot"></b>` : ""}</span></span>`).join("");
const OPPS = [
...
if (root) root.innerHTML = `<div class="opps"><div class="opps__compact"><span class="opps__compact-title">Opportunities</span><button class="opps__compact-avatar" type="button" aria-label="Profile"><img src="https://i.pravatar.cc/64?img=13" alt=""/></button></div><div class="opps__header"><span class="opps__title">Opportunities</span><span class="opps__avatar"><img src="https://i.pravatar.cc/96?img=13" alt="Profile"/></span></div><div class="opps__kpi">4/32 closed <b>+2%</b></div><div class="opps__bars" role="img" aria-label="Activity last 56 days, scrollable — outline gem marks achievements, filled gem the highest so far">${bars}</div><div class="opps__delta"><b>↑ +32 than yesterday</b> &nbsp;|&nbsp; last updated 4:35pm today</div><div class="opps__map" id="nearby-map">${mapViewHTML()}</div><div class="opps__filters"><button class="opps__tune" type="button" aria-label="Filters"><i class="ph ph-sliders-horizontal"></i></button><button class="opps__chip opps__chip--hot" type="button"><span class="opps__mini-stack"><i><img src="https://i.pravatar.cc/44?img=5" alt=""/></i><i><img src="https://i.pravatar.cc/44?img=44" alt=""/></i><i><img src="https://i.pravatar.cc/44?img=68" alt=""/></i></span>4 <i class="ph ph-flame opps__flame"></i></button><button class="opps__chip" type="button">Value range</button><button class="opps__chip" type="button">Stages</button><button class="opps__chip" type="button">Nearby</button></div><div class="opps__list">${cards}</div></div>`; initMap();
...
device?.classList.add("device--large");
const large = root?.querySelector(".opps__title"), small = root?.querySelector(".opps__compact-title");
const compact = root?.querySelector(".opps__compact");
const largeAv = root?.querySelector(".opps__avatar"), smallAv = root?.querySelector(".opps__compact-avatar");
/* iOS large-title morph: scroll progress drives the large title onto the fixed slot
   (scale + glide + crossfade); avatars crossfade. Measured live so type scale holds. */
...
const measure = () => { const lw = large.offsetWidth, lh = large.offsetHeight;
...
const frame = () => { tick = false; const y = feed?.scrollTop || 0;
  ...
  largeAv.style.opacity = 1 - smooth(0.4, 0.9, p); smallAv.style.opacity = q;
