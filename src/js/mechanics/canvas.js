...
export function createMechanicsCanvas(canvas, tooltip){
  ...
  const S={W:0,H:0,dpr:1,scale:1,ox:0,oy:0,fitted:false,dragGroup:null,dragNode:null,pan:null,hover:null,hoverGroup:null,selected:null,hovEdge:null,selEdge:null,downX:0,downY:0,moved:false,pending:false};
  const PAD=32;
  ...
  function fitScale(){ const b=getBB(); return clampScale(Math.min(1,(S.W-PAD*2)/b.w,(S.H-PAD*2)/b.h)); }
  function fitView(){ S.scale=fitScale(); const b=getBB(); S.ox=(S.W-b.w*S.scale)/2-b.minX*S.scale; S.oy=(S.H-b.h*S.scale)/2-b.minY*S.scale; S.fitted=true; }
  // keep graph spanning the container: center when smaller, clamp edges when larger
  function clampView(){ const b=getBB(), w=b.w*S.scale, h=b.h*S.scale, x1=b.minX+b.w, y1=b.minY+b.h;
    if(w+PAD*2<=S.W) S.ox=(S.W-w)/2-b.minX*S.scale; else S.ox=Math.max(S.W-PAD-x1*S.scale,Math.min(PAD-b.minX*S.scale,S.ox));
    if(h+PAD*2<=S.H) S.oy=(S.H-h)/2-b.minY*S.scale; else S.oy=Math.max(S.H-PAD-y1*S.scale,Math.min(PAD-b.minY*S.scale,S.oy)); }
  function resize(){ const r=canvas.parentElement.getBoundingClientRect(); if(!r.width||!r.height) return; S.dpr=Math.min(window.devicePixelRatio||1,2); S.W=r.width;S.H=r.height; canvas.width=S.W*S.dpr; canvas.height=S.H*S.dpr; canvas.style.width=S.W+'px'; canvas.style.height=S.H+'px'; ctx.setTransform(S.dpr,0,0,S.dpr,0,0); if(!S.fitted) fitView(); else clampView(); draw(); }
  ...
  function setMode(next){ if(!['decisions','schema','logic'].includes(next)) return; if(getMode()===next) return; setGraphMode(next); clearRouteCache(); S.selected=null; S.selEdge=null; S.hover=null; S.hovEdge=null; S.hoverGroup=null; tooltip.hidden=true; if(S.W&&S.H) fitView(); draw(); }
  ...
  canvas.addEventListener('keydown',e=>{
    ...
    if(e.key==='Enter'){ if(S.selected) tip(S.selected); e.preventDefault(); return; }
  ...
  attachCanvasInteraction({canvas,tooltip,S,getBB,world,hitNode,hitGroup,layoutGroup,reorderNode,draw,tip,tipEdge,hitEdge,setMode,clampView,fitView});
  window.addEventListener('resize',resize);
  try{ new ResizeObserver(()=>resize()).observe(canvas.parentElement); }catch{} resize();
